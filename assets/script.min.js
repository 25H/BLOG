...
var showPhotos=function(){var selector='.kratos-post-content img:not(.rolla-face), .kratos-status-post img:not(.rolla-face)';$(selector).off('click').on('click',function(e){var json={"data":[{"alt":$(this).attr('alt'),"src":$(this).attr('src'),}]}
layer.photos({photos:json,anim:0,});});}
var offcanvas=function(){var $clone=$('#kratos-menu-wrap').clone();$clone.attr({'id':'offcanvas-menu'});$clone.find('> ul').attr({'class':'ul-me','id':''});$('#kratos-page').prepend($clone);$('.js-kratos-nav-toggle').on('click',function(){if($('.nav-toggle').hasClass('toon')){$('.nav-toggle').removeClass('toon');$('#offcanvas-menu').css('right','-240px');}else{$('.nav-toggle').addClass('toon');$('#offcanvas-menu').css('right','0px');}});$('#offcanvas-menu a').on('click',function(){$('.nav-toggle').removeClass('toon');$('#offcanvas-menu').css('right','-240px');});$('#offcanvas-menu').css('height',$(window).height());$('#offcanvas-menu').css('right','-240px');$(window).resize(function(){var w=$(window);$('#offcanvas-menu').css('height',w.height());if(w.width()>769){if($('.nav-toggle').hasClass('toon')){$('.nav-toggle').removeClass('toon');$('#offcanvas-menu').css('right','-240px');}}});}
var menu=function(){$(document).click(function(e){var container=$('#offcanvas-menu,.js-kratos-nav-toggle');if(!container.is(e.target)&&container.has(e.target).length===0){if($('.nav-toggle').hasClass('toon')){$('.nav-toggle').removeClass('toon');$('#offcanvas-menu').css('right','-240px');}}});$('#kratos-header-section:not(.color-banner) ul>li').hover(function(){$(this).children('ul').slideDown(150)},function(){$(this).children('ul').stop(true,false).slideUp(200)});}
var donateConfig=function(){$(document).on('click','.Donate',function(){layer.open({type:1,area:['300px','370px'],title:'打赏作者',resize:false,scrollbar:false,content:'<div class="donate-box"><div class="meta-pay text-center"><strong>扫一扫支付</strong></div><div class="qr-pay text-center"><img class="pay-img" id="alipay_qr" src="/images/reward/zfb.png"><img class="pay-img d-none" id="wechat_qr" src="/images/reward/wx.png"></div><div class="choose-pay text-center mt-2"><input id="alipay" type="radio" name="pay-method" checked><label for="alipay" class="pay-button"><img src="/images/reward/alipay.png"></label><input id="wechatpay" type="radio" name="pay-method"><label for="wechatpay" class="pay-button"><img src="/images/reward/wechat.png"></label></div></div>'});$('.choose-pay input[type="radio"]').click(function(){var id=$(this).attr('id');if(id=='alipay'){$('.qr-pay #alipay_qr').removeClass('d-none');$('.qr-pay #wechat_qr').addClass('d-none')};if(id=='wechatpay'){$('.qr-pay #alipay_qr').addClass('d-none');$('.qr-pay #wechat_qr').removeClass('d-none')};});});}
var xControl=function(){$(document).on('click','.xHeading',function(event){var $this=$(this);$this.closest('.xControl').find('.xContent').slideToggle(300);if($this.closest('.xControl').hasClass('active')){$this.closest('.xControl').removeClass('active');}else{$this.closest('.xControl').addClass('active');}
event.preventDefault();});}
$.fn.pjaxReload=function(){sidebaraffix();showPhotos();if(typeof _ar!="undefined"){_ar.pjaxReload();}}
$(function(){gotop();sidebaraffix();offcanvas();menu();toSearch();showPhotos();donateConfig();xControl();});}());hljs.initHighlightingOnLoad();;var ajaxignore_string=new String('data:, javascript:, .pdf, .zip, .rar, .7z, .jpg, .png, .gif, .bmp');$(document).on("click","a[target!=_blank]",function(){if($(this).attr("href")){var req_url=$(this).attr("href")
if(!ajaxcheck_do(req_url)){return true}else{ajax(req_url,"pagelink")}}
return false});$(function(){window.addEventListener("popstate",function(e){if(e.state){document.title=e.state.title;$("#container").html(e.state.html);window.load=$(this).pjaxReload()}})});var ajaxignore=ajaxignore_string.split(', ');function ajaxcheck_do(url){if(!url)return false;for(var i in ajaxignore){if(url.indexOf(ajaxignore[i])>=0){return false}}
return true}
function ajax(reqUrl,msg,getData){var pageAnchor='',pageAnchorIndex=reqUrl.indexOf("#");if(msg=="pagelink"){}
if(pageAnchorIndex==-1){$("body,html").animate({scrollTop:$("#kratos-blog-post").offset().top-document.getElementById("kratos-header-section").offsetHeight},500)}else if(pageAnchorIndex==0){$("html, body").animate({scrollTop:$(reqUrl).offset().top-61},500);return}else if(pageAnchorIndex!=-1){pageAnchor=reqUrl.substring(pageAnchorIndex);reqUrl=reqUrl.substring(0,pageAnchorIndex);if(reqUrl==location.pathname){$("html, body").animate({scrollTop:$(pageAnchor).offset().top-61},500);return}}
NProgress.start();$("#container").fadeTo("fast",0.7);$.ajax({url:reqUrl,type:"GET",data:getData,beforeSend:function(){history.replaceState({url:window.document.location.href,title:window.document.title,html:$(document).find("#container").html(),},window.document.title,document.location.href)},success:function(data){reqUrl=reqUrl+pageAnchor;document.title=$(data).filter("title").text();window.history.pushState({url:reqUrl,title:$(data).filter("title").text(),html:$(data).find("#container").html()},$(data).filter("title").text(),reqUrl);if(msg=="pagelink"){$("#container").html($(data).find("#container").html());try{$("pre>code").each(function(){hljs.highlightBlock(this)})}catch(err){}}
$("#container").fadeTo("fast",1);if(pageAnchor!=""){$("html, body").animate({scrollTop:$(pageAnchor).offset().top-61},500);}
window.load=$(this).pjaxReload();NProgress.done()},error:function(request){$("#container").fadeTo("fast",1);location.href=reqUrl}})}
...
return NProgress.inc(0.3+0.5*Math.random()).set(1)};NProgress.inc=function(amount){var n=NProgress.status;if(!n){return NProgress.start()}else{if(n>1){return}else{if(typeof amount!=="number"){if(n>=0&&n<0.2){amount=0.1}else{if(n>=0.2&&n<0.5){amount=0.04}else{if(n>=0.5&&n<0.8){amount=0.02}else{if(n>=0.8&&n<0.99){amount=0.005}else{amount=0}}}}}
...
var i=cssPrefixes.length,capName=name.charAt(0).toUpperCase()+name.slice(1),vendorName;while(i--){vendorName=cssPrefixes[i]+capName;if(vendorName in style){return vendorName}}
return name}
...
return NProgress});;var ___as='屁屁..',_ar=_ar||{};_ar={init:function(){layer.msg('注：域名已改为 blog.arae.cc');_ar.layout.init();_ar.analytics.init();this.console_output();},pjaxReload:function(){_ar.layout.init();var _url=window.location.pathname;if(typeof _hmt.push!='undefined'&&typeof _hmt.push!='undefined'){_hmt.push(['_trackPageview',_url]);_czc.push(['_trackPageview',_url,_url_referer]);}
_url_referer=window.location.pathname;},console_output:function(){var css_font='font-family:"Helvetica Neue", Helvetica, Arial, sans-serif;';console.log('%c 来啊，互相伤害啊~',css_font+'font-size:20px;color:#777;text-shadow: 1px 1px #bbb, -1px -1px #444;');var d1=new Date('2028-02-18').getTime()-new Date().getTime(),d2=Math.floor(d1/(24*3600*1000));console.log('%c             距离域名到期剩余 '+d2+' 天  ←_←',css_font+'font-size:15px;color:#777;');}};var _hmt=_hmt||[],_czc=_czc||[],_url_referer=window.location.pathname;_ar.analytics={init:function(){this.baidu();this.cnzz();},loadScript:function(name,url){$.ajax({url:url,type:'GET',timeout:1000*5,dataType:'script',cache:true,async:true,error:function(xhr,status,error){console.log('>_ ['+name+'] 加载失败');}});},baidu:function(){this.loadScript('百度统计','https://hm.baidu.com/hm.js?529846c70f17e04eeaf59511a1170cea');},cnzz:function(){this.loadScript('CNZZ','https://s13.cnzz.com/z_stat.php?id=1273121910&web_id=1273121910&async=1');}};_ar.is={index:function(){if(location.pathname=='/'){return true;}},postlist:function(){var _pathname=location.pathname;if(_pathname=='/'||_pathname.slice(0,6)=='/page/'){return true;}},post:function(){if(location.pathname.slice(0,6)=='/post/'){return true;}},categories:function(){var _pathname=location.pathname;if(_pathname=='/'||_pathname.slice(0,6)=='/page/'){return true;}},archives:function(){if(location.pathname.slice(0,10)=='/archives/'){return true;}},tags:function(){if(location.pathname.slice(0,6)=='/tags/'){return true;}},};_ar.layout={init:function(){_ar.layout.global.init();_ar.layout.post.init();},};_ar.layout.global={init:function(){this.header();this.main();this.footer();},header:function(){var _tmp='';if(('ar_layout_global_search_lock'in window)!=true){$('#searchform').on('submit',function(e){var keyword=$('#search[name="wd"]').val(),url='https://www.google.com/search?q=site:arae.cc/post/%20'+keyword;if(keyword==''){layer.msg('啥都没有搜个屁');return false;}else if(navigator.userAgent.indexOf('iPad')>-1||navigator.userAgent.indexOf('iPod')>-1||navigator.userAgent.indexOf('iPhone')>-1){location.href=url;}else{window.open(url,"_blank");}
return false;});ar_layout_global_search_lock=true;}},main:function(){var $containerTOC=$('#TableOfContents');if($containerTOC.length>0){if('ar_layout_post_toc_lock'in window){tocbot.refresh();}else{tocbot.init({tocSelector:'#TableOfContents',contentSelector:'.kratos-post-content',headingSelector:'h1, h2, h3, h4, h5, h6',headingsOffset:-250,scrollSmooth:false,ignoreSelector:'#tocIgnore',});ar_layout_post_toc_lock=true;}
if($containerTOC.html().length<=0){$('.widget_toc').css({'height':'auto','min-height':'0'});$containerTOC.text('这里什么都没有...');}}},footer:function(){}};_ar.layout.post={init:function(){this.post();},post:function(){if(_ar.is.post()!=true){return;}},};$(function(){if("ar_main_load_lock"in window)return;ar_main_load_lock=true;_ar.init();});function isArray(_var){return Object.prototype.toString.call(_var)=='[object Array]';}