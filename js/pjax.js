var ajaxignore_string = new String('data:, javascript:, .pdf, .zip, .rar, .7z, .jpg, .png, .gif, .bmp');
...
function ajax(reqUrl, msg, getData) {
    ...
    var pageAnchor = '',
        pageAnchorIndex = reqUrl.indexOf("#")
        ;
    
    if (msg == "pagelink") {/* ... */}
    
    if (pageAnchorIndex == -1) {
        $("body,html").animate({
            scrollTop: $("#kratos-blog-post").offset().top - document.getElementById("kratos-header-section").offsetHeight
        }, 500)
    } else if (pageAnchorIndex == 0) {
        // 滚动到描点位置
        console.log(reqUrl)
        $("html, body").animate({ scrollTop: $(reqUrl).offset().top - 61 }, 500);
        return
    } else if (pageAnchorIndex != -1) {
        // 跳转后滚动到描点位置
        pageAnchor = reqUrl.substring(pageAnchorIndex);
        reqUrl = reqUrl.substring(0, pageAnchorIndex);
        // =当前页
        if (reqUrl == location.pathname) {
            $("html, body").animate({ scrollTop: $(pageAnchor).offset().top - 61 }, 500);
            return
        }
    }
...
/* NProgress, (c) 2013, 2014 Rico Sta. Cruz - https://github.com/rstacruz/nprogress
 * @license MIT */
